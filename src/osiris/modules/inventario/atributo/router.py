...
def get_atributo(item_id: UUID = Path(...), session: Session = Depends(get_session)):
...
@router.post("", response_model=AtributoRead, status_code=status.HTTP_201_CREATED)
def create_atributo(payload: AtributoCreate = Body(...), session: Session = Depends(get_session)):
    return service.create(session, payload.model_dump(exclude_unset=True))


@router.put("/{item_id}", response_model=AtributoRead)
def update_atributo(
    item_id: UUID = Path(...),
...
def delete_atributo(item_id: UUID = Path(...), session: Session = Depends(get_session)):
